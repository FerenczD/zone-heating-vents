...
export class HomeSetupScreen extends Component {
    ...
    // testias = () => {
    //     fetch(this.state.url + '/venti/test.php', {
    //         method: 'POST',
    //         headers: {
            ...
            NetworkInfo.getBSSID().then(bssid => {
                ...
                SmartConfigP.start({
                    ...
                    if(data) {
                      ...
                      let timeout = setTimeout(() => { clearInterval(timerId); alert("Smart Config Timed Out")}, 10000);
    ...
    this.getHowManyVentsAlreadyPaired().then(response => {
        if(!isNaN(response.message)) {
            ...
            let timerId = setInterval(() => {this.checkIfVentWasPaired();
                if(this.state.ventPaired == true) {
                    clearInterval(timerId);
                    clearTimeout(timeout);
                }
            }, 2000);
            let timeout = setTimeout(() => { this.updateVentPairingFlag(false);clearInterval(timerId)}, 60000);
        }
        else {
    ...
    createRoom = () => {
        ...
        fetch(this.state.url + '/venti/createRoom.php', {
            ...
            if (res.message != '') {
                console.log('New vent with MAC Address: ' + res.message + ' found and paired!');
                this.setState({ventPaired: false, newVent: res.message,dialogAddNewVent: true, rooms: [...this.state.rooms, ventName],addNewVent: true}, ()=> {
                    console.log("states: " + this.state.dialogAddNewVent + "," + !this.state.dialogFoundVent + ","  + this.state.addNewVent + "," + this.state.ventPaired);
                });
            } else {
        ...
        return (
            <ScrollView>
                ...
                <Dialog.Container visible={this.state.ventPaired && this.state.dialogFoundVent}>
                    ...
                    <Dialog.Description>
                        New vent found with MAC address: {this.state.newVent} found and successfully paired! Give it a name.
                    </Dialog.Description>
                ...
                <Dialog.Container visible={this.state.dialogAddNewVent && !this.state.dialogFoundVent && this.state.addNewVent}>
                ...
                <View style={{flexDirection: 'row', paddingBottom: 10,  alignSelf: 'center', textAlign: 'center'}}>
                    <View style={{ alignSelf: 'center', textAlign:'center'}}>
                        <Text style={{fontWeight: 'bold', paddingTop: 20, paddingLeft:15}}>To begin setting up Venti, plug it in and activate the pairing button. Once ready, press Pair Thermostat to begin</Text>
                    </View>
                </View>
                <View style={{flexDirection: 'row', paddingBottom: 10,  alignSelf: 'center'}}>
                    <View style={{paddingBottom: 20, paddingTop:20, alignSelf: 'center', justifyContent: 'center', alignItems: 'center'}}>
                        <Button title="Pair Thermostat" onPress={this.smartConfig} style={{textAlign: 'center',
                            width: 200,
                            alignContent: 'center',
                ...
                <View style={{flexDirection: 'row',alignSelf:'center'}}>
                    <TextInput
                        style={{height: 40, borderColor: 'gray', borderWidth: 1, width: 250,backgroundColor: this.state.wifiPaired ? '#FFF' : '#EDEDED' }}
                        editable={this.state.wifiPaired}
                        onChangeText={text => {this.setState({thermostatName:text},()=>{
                            this.nameHome(text);
                        })
                        }}
                ...
                <View style={{flexDirection: 'row', paddingBottom: 10, alignSelf: 'center'}}>
                    <View>
                        <Text style={{fontWeight: 'bold', paddingTop: 80, paddingLeft:20}}>Once you're ready, press Pair Vent to begin setting up your vents.</Text>
                    </View>
                </View>
                <View style={{flexDirection: 'row', paddingBottom: 10,  alignSelf: 'center'}}>
                    <View style={{paddingBottom: 20, paddingTop:20, alignSelf: 'center', justifyContent: 'center', alignItems: 'center'}}>
                        <Button title="Pair Vent" onPress={this.pairNewVent} disabled={!this.state.wifiPaired} style={{textAlign: 'center',
                            width: 200,
                            alignContent: 'center',
                            justifyContent: 'center',
                            backgroundColor: 'navy',
